...
FrontEstimateX::FrontEstimateX()
{
  ...
  m_num_reports_posted = 0;
  m_returned = false;
  m_deployed = false;
  report_var = "UCTD_PARAMETER_ESTIMATE";
  spiral_report_var = "SPIRAL_REPORT_PARAMS";
}
...
bool FrontEstimateX::OnConnectToServer()
{
  ...
  Register("SEND_FINAL_ESTIMATES",0);
  Register("SPIRAL_REPORT_REQ",0);
...
bool FrontEstimateX::Iterate()
{
  ...
  // acquire the latest estimate from the annealer
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++)
    {
      ...
      else if (rMsg.m_sKey == "SPIRAL_REPORT_REQ")
      {
        m_num_reports_requested++;
        if (in_survey)
          postSpiralParameters();
      }
      else if (rMsg.m_sKey == "SEND_FINAL_ESTIMATES")
      {
        in_survey = false;
        completed = true;
        report_sent = true;
...
void FrontEstimateX::postParameterReport()
{
  // generate the report string to share
  string sval;
  sval = "vname=" + vname;
  ...
  Notify(report_var, sval);
}


void FrontEstimateX::postSpiralParameters()
{
  // generate the report string to share with the spiral moos app
  m_num_reports_posted++;
  string sval;
  sval = "vname=" + vname;
  sval += ",offset=" + doubleToString(offset);
  sval += ",angle=" + doubleToString(angle);
  ...
  sval += ",tempnorth=" + doubleToString(T_N);
  sval += ",tempsouth=" + doubleToString(T_S);
  Notify(spiral_report_var, sval);
}


bool FrontEstimateX::buildReport()
{
  m_msgs<<"FrontEstimateX report"<<endl;
  m_msgs<<"____________________"<<endl;
  double energy = anneal.heatBath(temperature);
  m_msgs<<"Annealing step =        "<<anneal_step-1<<", Energy = "<<energy<<endl;  
  m_msgs<<"Num Reports Posted =    " << m_num_reports_posted << endl;
  m_msgs<<"Num Reports Requested = " << m_num_reports_requested << endl;
  m_msgs<<"In Survey Mode =        " << in_survey << endl;
  m_msgs<<"Survey Completed =      " << completed << endl;
  m_msgs<<"Report Sent =           " << report_sent << endl;
  m_msgs<<"____________________"<<endl;
  m_msgs<<"Last Anneal Report:"<<endl;
